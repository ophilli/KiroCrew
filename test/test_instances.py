...
class TestSshTunnelManager:
    ...
    @pytest.mark.asyncio
    async def test_disabling_loopback_config_tears_down_live_tunnel(self, tmp_path, monkeypatch):
        from kiro_crew.config.loader import KiroCrewConfig
        ...
        monkeypatch.setattr(stm, "mint_loopback_token", mint)
        assert (await mgr.connect("lb-1")).state is TunnelState.CONNECTED
        tunnel = mgr._tunnels["lb-1"]
        assert mgr.get_token("lb-1") == "SECRET_TOK"

        _enable(tmp_path, monkeypatch, allow_loopback_transport=False)
        cleanup = mgr.apply_config(KiroCrewConfig.load().instances)
        assert cleanup is not None
        await cleanup

        assert tunnel.stopped is True
        assert mgr.status("lb-1") is None
        assert mgr.get_token("lb-1") == ""
        persisted = reg.get("lb-1")
        assert persisted is not None
        assert persisted.local_port == _UNALLOCATED_PORT
        assert persisted.was_connected is False

    @pytest.mark.asyncio
    async def test_connect_normalizes_stored_loopback_method_in_restart_hints(
        self, tmp_path, monkeypatch
    ):
        from kiro_crew.instances import ssh_tunnel_manager as stm
        from kiro_crew.instances.registry import (
        ...
        monkeypatch.setattr(stm, "mint_loopback_token", mint)
        status = await mgr.connect("lb-1")
        assert status.state is TunnelState.CONNECTED
        await mgr.shutdown()

        persisted = InstancesRegistry(reg.path).get("lb-1")
        assert persisted is not None
        assert persisted.connection_method == CONNECTION_METHOD_LOOPBACK
        assert persisted.was_connected is True
        assert persisted.local_port == status.local_port
...
class _LoopbackRecoveryTunnel(_ResilTunnel):
    """Fake retained loopback tunnel with scripted re-proof outcomes."""

    def __init__(self, *args, rearm_results, **kwargs):
        super().__init__(*args, transport="loopback", **kwargs)
        self.rearm_results = list(rearm_results)
        self.rearm_calls = 0
        self.rearm_started: asyncio.Event | None = None
        self.rearm_release: asyncio.Event | None = None

    async def rearm_loopback(self):
        self.rearm_calls += 1
        if self.rearm_started is not None:
            self.rearm_started.set()
        if self.rearm_release is not None:
            await self.rearm_release.wait()
        ok = self.rearm_results.pop(0)
        self.status.state = self._S.CONNECTED if ok else self._S.ERROR
        return ok
...
class TestSelfHealRefreshRestart:
    ...
    def _mgr(self, tmp_path, *, mint=None, factory=_ResilTunnel, allow_loopback=None):
        from kiro_crew.instances.registry import InstancesRegistry
        from kiro_crew.instances.ssh_tunnel_manager import SshTunnelManager
    ...
    @pytest.mark.parametrize("rearm_ok", [True, False])
    @pytest.mark.asyncio
    async def test_loopback_restore_bumps_epoch_once_for_either_result(
        self, tmp_path, monkeypatch, rearm_ok
    ):
        from kiro_crew.instances.ssh_tunnel_manager import TunnelState

        monkeypatch.setenv("KIROCREW_POD", "1")
        reg, mgr = self._mgr(tmp_path, allow_loopback=True)
        reg.add(
            name="LB",
            instance_id="lb-1",
            connection_method="loopback",
        ...
        inst = reg.get("lb-1")
        current = _LoopbackRecoveryTunnel("lb-1", "", 53999, 7901, rearm_results=[rearm_ok])
        current.status.state = TunnelState.ERROR
        mgr._tunnels["lb-1"] = current
        mgr._tunnel_epoch["lb-1"] = 7

        restored = await mgr._restore_for_recovery(
            inst,
        ...
        assert current.rearm_calls == 1
        assert mgr._tunnel_epoch["lb-1"] == 8

    @pytest.mark.asyncio
    async def test_loopback_rearm_stands_down_when_the_epoch_moves(self, tmp_path, monkeypatch):
        from kiro_crew.instances.ssh_tunnel_manager import TunnelState
        ...
        current = _LoopbackRecoveryTunnel("lb-1", "", 53999, 7901, rearm_results=[True])
        current.status.state = TunnelState.ERROR
        current.rearm_started = asyncio.Event()
        current.rearm_release = asyncio.Event()
        mgr._tunnels["lb-1"] = current
        mgr._tunnel_epoch["lb-1"] = 11
        writes = []
        ...
        recovery = asyncio.create_task(mgr._recover("lb-1"))
        await asyncio.wait_for(current.rearm_started.wait(), timeout=5)
        async with mgr._lock:
            mgr._tunnel_epoch["lb-1"] = 12
        current.rearm_release.set()
        await asyncio.wait_for(recovery, timeout=5)

        assert current.rearm_calls == 1
        assert mgr._tunnel_epoch["lb-1"] == 12
        assert mgr._recover_attempts["lb-1"] == 1
        assert writes == []

    @pytest.mark.asyncio
    async def test_loopback_tier2_uses_the_failed_tier1_epoch(self, tmp_path, monkeypatch):
        from kiro_crew.instances.ssh_tunnel_manager import TunnelState
        ...
        current = _LoopbackRecoveryTunnel("lb-1", "", 53999, 7901, rearm_results=[False, True])
        current.status.state = TunnelState.ERROR
        mgr._tunnels["lb-1"] = current
        mgr._tunnel_epoch["lb-1"] = 20
        mint_epochs = []

        async def mint(_inst, _params):
            mint_epochs.append(mgr._tunnel_epoch["lb-1"])
            return "TOK-LOOPBACK"

        monkeypatch.setattr(mgr, "_mint_for", mint)
        try:
            await mgr._recover("lb-1")

            assert mint_epochs == [21]
            assert current.rearm_calls == 2
            assert mgr._tunnel_epoch["lb-1"] == 22
            assert mgr._tunnels["lb-1"] is current
            assert mgr.get_token("lb-1") == "TOK-LOOPBACK"
            assert mgr._recover_attempts.get("lb-1", 0) == 0
            assert reg.get("lb-1").was_connected is True
        finally:
            mgr._cancel_token_refresh("lb-1")
...
class TestForwarderPidHints:
    ...
    async def test_both_identity_writes_carry_the_same_fields(self, tmp_path, monkeypatch):
        ...
        # connect adds its own extras and nothing else. The canonical method is
        # persisted here so a raw stored spelling cannot make hint validation
        # discard the successful connection state.
        assert set(connect_kwargs) == identity_fields | {
            "connection_method",
            "mark_last_active",
        }
        assert connect_kwargs["connection_method"] == "ssh"
...
class TestLoopbackBrowserPortIsHubOwned:
    ...
    def test_the_pane_port_normalizes_stored_loopback_destinations(self, tmp_path, monkeypatch):
        """A stored method spelling still reserves the sibling's destination."""
        from kiro_crew.instances import ssh_tunnel_manager as stm
        from kiro_crew.instances.registry import InstancesRegistry

        monkeypatch.setenv("KIROCREW_HOME", str(tmp_path))
        registry_path = tmp_path / "instances.json"
        registry = InstancesRegistry(registry_path)
        mgr = stm.SshTunnelManager(registry, allow_loopback=True)
        sibling_port = mgr._allocator.base_port
