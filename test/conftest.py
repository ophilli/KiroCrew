...
def assert_rejected_without_backtracking(reject, build_pump, *, samples: int = 5) -> None:
    ...
    * **Minimum over samples, with an early exit.** A gen-2 garbage collection
      charged to this thread mid-search is the one thing that can still spend CPU
      here; it cannot hit two consecutive samples, so two over-budget samples are
      a verdict and the measurement stops rather than paying five regressions.
