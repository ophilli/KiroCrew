 ...
 *   - `refused` REJECTS (nothing is running; the page's failWith reports it
 *     and drops the pending critique, as it did for a non-2xx); a fetch
 *     rejection is `indeterminate()` on this wire, so `transport-error` has no
 *     producer here;
...
describe('designCritiqueApi.send — the transport receipt decides', () => {
  ...
  it('resolves when the fetch itself fails — indeterminate, never a retry-safe failure', async () => {
    // "Never left" and "the gateway took the POST and the connection reset
    // before headers" are the same rejection from here; the second means a
    // retry runs the critique twice, so the wire says indeterminate and the
    // poll that follows settles it (the Opus finding this pins).
