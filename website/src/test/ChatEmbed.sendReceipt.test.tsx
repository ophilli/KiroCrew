 ...
 * - the unconfirmed notice on a chip send says "tap the suggestion again" instead
...
describe('ChatEmbed send receipt policy', () => {
  ...
  it("a follow-up chip's send that times out says to tap the suggestion again, not that the text is back", async () => {
    mockGet.mockResolvedValue({
      messages: [{ role: 'assistant', content: 'Next? [OPTIONS: Run tests | Skip]' }],
      running: false,
      title: '',
    ...
    await act(async () => { await vi.advanceTimersByTimeAsync(SEND_ABORT_MS + 50) })
    expect(noticeRows()).toHaveLength(1)
    expect(noticeRows()[0].textContent).toMatch(/tap the suggestion again/)
