 ...
 * - unknown (unreadable 2xx)     -> the response-late recovery: bubble rolled
 *                                  back, text handed back under the notice
 *                                  (the outage that cut the body can also
 *                                  swallow the SSE frame that clears pending).
...
vi.mock('../api/client', () => ({
  api: {
    ...
    // The footer's read-only-tools note (#9609) reads the loaded config; a
    // rejected load renders an ErrorNotice (role=alert) that would collide
    // with the send-receipt alerts these tests assert on.
...
describe('SideChat send receipt policy', () => {
  ...
  it('a late response on a CHIP send says to tap the suggestion again and leaves the draft alone', async () => {
    vi.useFakeTimers()
    vi.mocked(api.sideTurn).mockImplementationOnce(() => new Promise(() => {}) as ReturnType<typeof api.sideTurn>)
    ...
    expect(screen.getByText(/tap the suggestion again/)).toBeInTheDocument()
  ...
  it('an accepted /side/turn whose body is unreadable is UNKNOWN: not an error, but the bubble rolls back and the text comes back under the unconfirmed notice', async () => {
    const user = userEvent.setup()
    ...
    // Never a failure: the server probably has the question, so no red alert
    // and no "try again".
    await waitFor(() => expect(screen.getByText(/^Delivery not confirmed/)).toBeInTheDocument())
    expect(screen.queryByRole('alert')).toBeNull()
    // But the panel must not be left pending on a turn whose lifecycle frames
    // the same outage may have swallowed: the bubble rolls back, the text is
    // handed back, and `pending` is clear (the GPT finding this pins).
    expect(box()).toHaveValue('accepted but mangled')
    expect(sideMessages(store).some(m => m.content === 'accepted but mangled')).toBe(false)
    expect(store.getState().chat.slotSide[SLOT]?.pending ?? false).toBe(false)
  })
  ...
  it('a late response on an IDLE send rolls the bubble back and hands the text back under the unconfirmed notice (the panel is never left pending for a turn the server may not have)', async () => {
    vi.useFakeTimers()
    vi.mocked(api.sideTurn).mockImplementationOnce(() => new Promise(() => {}) as ReturnType<typeof api.sideTurn>)
    const store = sideState(false)
    renderWithProviders(<SideChat slot={SLOT} />, { store })
    ...
    await act(async () => { await vi.advanceTimersByTimeAsync(SEND_ABORT_MS + 50) })
    // The bubble is gone, the text is back, the standing notice explains, and
    // the panel is not stuck busy: no poll exists here that could ever clear a
    // `pending` the server never confirmed (the GPT finding this pins).
    expect(sideMessages(store).some(m => m.content === 'slow one')).toBe(false)
    expect(box()).toHaveValue('slow one')
    expect(screen.getByText(/^Delivery not confirmed/)).toBeInTheDocument()
    expect(store.getState().chat.slotSide[SLOT]?.pending ?? false).toBe(false)
  })
})
