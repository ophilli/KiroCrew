...
describe('private member memory controls', () => {
  ...
  it('keeps a legacy member on usable V1 until its owner confirms creating V2', async () => {
    ...
    const hint = screen.getByText(/Uses the current shared memory \(V1\), not a private store\./)
    expect(hint).toHaveTextContent(/^Uses the current shared memory \(V1\), not a private store\.$/)
  ...
  it.each([
    ...
    expect(screen.queryByText(/Uses the current shared memory \(V1\), not a private store\./)).toBeNull()
