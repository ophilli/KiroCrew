...
/**
 * The catalog glues the limit to the word before it with U+00A0. A wrapped
 * "Below your limit of" leaving a bare "14" on the next line puts back exactly
 * the loose second number this line exists to remove, and the column is a third
 * of the card wide, so the break is reachable.
 */
const NBSP = ' '
...
describe('TasksCapacityCard', () => {
  ...
  it('shows depth, oldest wait, cap vs ceiling, the degrade reason and each wait with its age', async () => {
    ...
    // The cap in force as a short headline, and beneath it the RELATIONSHIP to
    // the user's own ceiling — never a second bare maximum, which reads as two
    // unrelated answers to one question, and never "N of M" beside "0 running".
    ...
    expect(subagentLines).toEqual(['Up to 4 at once', `Below your limit of${NBSP}8`, '3 running now'])
    const spawnGateLines = Array.from(spawnGate.querySelectorAll('span')).map(s => s.textContent)
    expect(spawnGateLines).toEqual(['Up to 2 at once', `Below your limit of${NBSP}8`])
  ...
  it('says why a lane is below the limit, and shows one number when it is not', async () => {
    ...
    // Lowered: the relationship to the reader's own ceiling AND the cause, in the
    // same words the degrade row uses — one cause with two wordings reads as two.
    expect(linesOf(subagents)).toEqual([
      'Up to 2 at once', `Below your limit of${NBSP}14`, 'Concurrency lowered under load', '2 running now',
    ])
    ...
    // Each line beneath the headline is a SENTENCE about a number: the UI font,
    // and free to wrap. A column is a third of the card, so an unbreakable
    // phrase this long lands on top of the column beside it.
    const subagentLines = Array.from(subagents.querySelectorAll('span'))
    expect(subagentLines[0].className).toContain('whitespace-nowrap')
    for (const line of subagentLines.slice(1)) {
  ...
  it('prints one cause once when TWO lanes are lowered, in the row and on neither lane', async () => {
    ...
    const [subagents, spawnGate] = screen.getAllByTestId('tasks-capacity-lane')
    const linesOf = (el: Element) => Array.from(el.querySelectorAll('span')).map(s => s.textContent)
    expect(linesOf(subagents)).toEqual(['Up to 2 at once', `Below your limit of${NBSP}14`, '2 running now'])
    expect(linesOf(spawnGate)).toEqual(['Up to 2 at once', `Below your limit of${NBSP}8`])
  })
  ...
  it('renders no degrade row when nothing lowered the cap, stalled badge included', async () => {
    ...
    expect(screen.getByText('Stalled', { selector: 'span:not([data-testid])' })
      .closest('div')!.textContent).toContain('1')
  })
