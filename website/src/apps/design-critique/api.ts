...
import { indeterminate, sendTurn, type SendWire } from '../../chat-core/transport/sendTurn'
 ...
 *  the transport's deadline signal. A rejected fetch is INDETERMINATE, per the
 *  `SendWire` contract: "never left" and "the gateway took the POST and the
 *  connection reset before headers" are the same rejection from here, and the
 *  second means a retry runs the critique twice -- so the wire says
 *  `indeterminate()` (→ `response-late`, which `send` resolves and the poll
 *  settles) rather than passing the raw error through as a retry-safe
 *  `transport-error`. This app has no optimistic bubble and no composer, so
 *  the dashboard wire's exemption does not apply to it. */
...
export const designCritiqueApi = {
  ...
  // `refused` rejects (nothing is running -- the caller's failWith reports it
  // and drops the pending critique, as it did for a non-2xx before); `unknown`
  // and `response-late` RESOLVE, because the request was or may have been
  // accepted and the poll that follows will find out -- rejecting would invite
  // a retry that runs the critique twice. `transport-error` has no producer
  // here: `critiqueSendWire` converts every rejection to `indeterminate()`. The old send read the SSE
