...
vi.mock('../../api/client', () => ({
  api: {
    ...
    // The empty crew asks the registry whether there is anyone to hire; the
    // default answers "only `default`" so the plain empty state is what every
    // other case sees.
    kirocrewAgents: vi.fn(() => Promise.resolve({ agents: [{ name: 'default' }] })),
  },
}))
...
beforeEach(() => {
  ...
  vi.mocked(api.kirocrewAgents).mockImplementation(() => Promise.resolve({ agents: [{ name: 'default' }] }))
...
describe('MembersPage side panel (Crew summary tab) and edit jump', () => {
  ...
  it('does not describe a legacy shared named store as private V2', async () => {
    ...
    expect(drawer).not.toHaveTextContent('Private memory (V2) starts empty in a new chat')
    expect(within(drawer).getByRole('button', { name: 'Set up private memory' })).toBeVisible()
    expect(drawer).not.toHaveTextContent(/used only by its owner/i)
  })
  ...
  it('the empty roster names the hire path for existing rows only when the registry holds more than `default`', async () => {
    // Fresh install: `default` alone -- the plain empty state, no pointer.
    const fresh = await renderPage([])
    await screen.findByTestId('member-empty-cta')
    await waitFor(() => expect(api.kirocrewAgents).toHaveBeenCalled())
    expect(screen.queryByTestId('member-empty-templates')).toBeNull()
    fresh.unmount()
    // Upgrader (or a package's agents): the rows are templates now, and the
    // line says so -- and says the CTA is how they come back.
    vi.mocked(api.kirocrewAgents).mockImplementation(() =>
      Promise.resolve({ agents: [{ name: 'default' }, { name: 'Nia' }] }),
    )
    await renderPage([])
    const line = await screen.findByTestId('member-empty-templates')
    expect(line).toHaveTextContent(/listed as Agent templates/)
    expect(line).toHaveTextContent(/hiring one by name/)
  })
