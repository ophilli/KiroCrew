  ...
  return (
    <div ref={rowRef} className={`p-2 rounded-md border ${autoOpen ? 'border-accent ring-1 ring-accent' : 'border-border'}`}>
      <div className="flex items-center gap-3">
        ...
        <Btn
          /* Non-primary while the candidate is flagged: a bright primary button
             beside "approving will be refused" reads as a mixed signal (UX blind
             read: "it looks pressable, but the banner says pressing it won't
             work"). It stays ENABLED — the server is the authority and a fixed
             script can be retried — and the hint below the row states that in
             visible text, reachable by touch/keyboard, not only on hover. */
          primary={p.script_validation?.ok !== false}
          disabled={!open || !detail || (isUpdate && (!detail.diff || !!detail.stale_base))}
          title={p.script_validation?.ok === false ? i18nT('pages.overview.skillsTab.approve_flagged_hint') : undefined}
...
function PendingSkillsPanel() {
  ...
  const [panelError, setPanelError] = useState<{ message: string; journal?: ReturnType<typeof findReport> } | null>(null)
  ...
  const dismiss = useMutation({
    ...
    onError: (err) => {
      setPanelError({
        ...
        // Keyed by the ORIGINAL message apiFailure journaled, not the
        // localized one we display (same contract as the approve refusal).
        journal: findReport(err instanceof Error ? err.message : undefined),
      })
    },
    onSuccess: (_data, slug) => {
