...
export function useAvailableModelsQuery({ enabled }: AvailableModelsOptions = {}) {
  ...
  // Applied HERE (not only in `useAvailableModels`) because ChatPanel reads
  // this query variant directly for `isDegraded`, and its arrange-your-models
  // card must show rows in the same saved order every picker renders.
  ...
  const list = query.data ?? PLACEHOLDER
  const ordered = useMemo(() => applyModelOrder(list, order ?? []), [list, order])
  return { ...query, data: ordered, isDegraded }
}
