 ...
 *
 * Query: ?theme=dark|light  ?refuse=409|403|late  ?chips=1
 ...
 *   chips = the last assistant row ends in an `[OPTIONS: ...]` footer, so the
 *          FollowUpBar renders its chips. A chip's "send now" segment sends the
 *          option WITHOUT consuming the draft, so its late notice says
 *          "re-pick the option" (`delivery_unconfirmed_option`) instead of
 *          claiming a restore that never happened.
 */
...
const chips = params.get('chips') === '1'
...
const transcript = {
  ...
  messages: [
    ...
    {
      role: 'assistant',
      content: 'Three criteria: a 429 on the 11th upload within a minute, a `Retry-After` header on that 429, and the limit applied per API key rather than per IP.'
        // The footer the agent's own protocol uses to offer follow-ups; the
        // chip scene photographs a send that originates from one of them.
        + (chips ? '\n\n[OPTIONS: Add a per-IP fallback | Write the tests first]' : ''),
      cls: 'msg msg-a',
      ts: '2026-09-04T18:00:04Z',
    },
  ],
}
