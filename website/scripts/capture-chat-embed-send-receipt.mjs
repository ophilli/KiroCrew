 ...
 *  - chip-late: the same deadline, but the send came from a follow-up chip's
 *               "send now" segment. A chip send never consumed the draft, so
 *               the notice says to tap the suggestion again instead of claiming a
 *               restore -- and the draft the user was typing is left alone.
 *
...
// The chip scene: the transcript ends in an [OPTIONS: ...] footer, the user has
// a half-typed draft, and sends an option through the chip's "send now" segment.
// Asserted: the notice is the chip variant ("tap the suggestion again"), and the draft
// is untouched -- the chip never consumed it, so nothing was restored into it.
const DRAFT = 'Also cap the per-'
const OPTION = 'Add a per-IP fallback'
for (const theme of ['dark', 'light']) {
  await page.goto(`${BASE}/capture/chat-embed-send-receipt.html?theme=${theme}&refuse=late&chips=1`, { waitUntil: 'networkidle' })
  const input = page.getByLabel('Chat message', { exact: true })
  await input.waitFor()
  await input.fill(DRAFT)
  await page.getByRole('button', { name: `Send now: ${OPTION}` }).click()
  const ROW_RE = /Delivery not confirmed/
  await page.waitForFunction((re) => {
    const nodes = Array.from(document.querySelectorAll('[data-capture-root] *'))
    return nodes.some(n => n.childElementCount === 0 && new RegExp(re).test(n.textContent || ''))
  }, ROW_RE.source, { timeout: 15000 }).catch(() => {})
  ...
  const composer = await input.inputValue()
  console.log(`${theme}/chip-late: row="${rowText}" composer="${composer}"`)
  if (!/tap the suggestion again/.test(rowText)) { console.error(`FAIL: ${theme}/chip-late expected the chip-send notice, got "${rowText}"`); failures++ }
  if (composer !== DRAFT) { console.error(`FAIL: ${theme}/chip-late the draft must be left alone (got "${composer}")`); failures++ }
  await page.evaluate(() => Promise.all(document.getAnimations().map(a => a.finished)))
  await page.screenshot({ path: `${OUT}/chip-late-${theme}.png` })
}
