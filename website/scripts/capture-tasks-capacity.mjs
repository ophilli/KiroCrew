 ...
 * retry"; `degraded` for the mapped degrade reason on the lane line and its warn
 * badge; `stalled` for the red badge with no degrade row beside it; `folded` for
 * the "Show N more" control past `WAIT_ROWS_SHOWN` and `expanded` for what that
 * control opens; `narrow` for the 320px stack and `narrow-folded` for that stack
 * at the height a busy queue actually reaches; and `no-store` / `error` for the
 * two arms that render a notice instead of a queue — the second with a body that
 * carries no readable reason, which is what the sentence has to survive.
 *
...
const SHOTS = [
  ...
  { name: 'tasks-capacity-no-store', payload: NO_STORE },
  // The load-error arm renders its own notice and never mounts the badge, so
  // it waits on a different element than every other scene.
...
async function frameCard(page, card, name, { pill }) {
  ...
  console.log(`wrote ${path} ${width}x${height}`
    ...
    + ` · health pill ${geom.hasPill ? 'in frame, unoccluded' : 'absent (the load-error arm mounts none)'}`)
}
...
try {
  for (const { name, payload, viewport, status, ready, click } of SHOTS) {
    ...
    // Every scene but the load-error arm waits on the badge itself, so `ready`
    // being set IS the scene saying it renders no pill.
