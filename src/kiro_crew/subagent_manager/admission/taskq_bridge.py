...
class _TaskqBridgeMixin(ManagerComponent):
    ...
    def taskq_refill_window(self, *, children_only: bool = False) -> int:
        ...
        try:
            absent = self._refill_absent(store, children_only)
            room, want = self._refill_make_room(store, absent)
    ...
    async def taskq_refill_window_async(self, *, children_only: bool = False) -> int:
        ...
        rows: list[_taskq.TaskRecord] = []
        try:
            absent = await store.run(self._refill_absent, store, children_only)
            room, want = self._refill_make_room(store, absent)
    ...
    def _refill_absent(self, store: "_taskq.TaskStore", children_only: bool) -> list[str]:
        """Store read: lanes with a waiting row whose head is not in the window.
        (The window's lane set is read first, on whatever thread calls.)"""
        from kiro_crew import taskq as _taskq

        present = {
            self.lane_of_entry(p)
        ...
        return [lane for lane in pending if lane not in present]

    def _refill_make_room(self, store: "_taskq.TaskStore", absent: list[str]) -> tuple[int, int]:
        ...
        if absent:
            self._evict_for_lanes(want - (store.window - len(queue)))
    ...
    def _evict_for_lanes(self, count: int) -> int:
        ...
        the window keeps its head and its FIFO order.
        """
        queue = self._manager._queue
        evicted = 0
        while evicted < count:
            ...
            for idx, params in enumerate(queue):
                if params.get("_resume_id") or not params.get("_preassigned_id"):
                    continue
                by_lane.setdefault(self.lane_of_entry(params), []).append(idx)
