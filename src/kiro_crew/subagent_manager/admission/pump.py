...
class _PumpMixin(ManagerComponent):
    ...
    async def _drain_queue_pass_impl(self) -> None:
        ...
        try:
            ...
            self._drain_queue_sync_impl(
                refill=lambda **_kw: 0, dispatch=picked.append, grant=granting.append
            )
    ...
    def _drain_queue_sync_impl(
        ...
        *,
        ...
        writer thread and publishes the run state from the result)."""
        ...
        index = self._manager._admission.pick_window_index(view)
        if index is None:
