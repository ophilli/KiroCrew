...
class RunEventCoordinator(ManagerComponent):
    ...
    def _dependency_report_db(
        ...
        The mark is ``TaskStore.advance``, the same entry point ``taskq_advance``
        uses, and never a bare ``transition``: an ``admitted`` row here (its
        ``starting`` write lost to a locked database) would otherwise refuse this
        mark, then the wait, then the park -- three ``rejected_transition``
        events leaving a LIVE run parked on a dependency backoff behind a row the
        next boot requeues blind.
        """
        from kiro_crew import taskq as _taskq

        if mark_running and store is not None:
            try:
    ...
    async def _dependency_verdict(
        ...
        without it, inline, where program order already holds.
        """
        admission = self._manager._admission
        store = admission.taskq_store()
        mark = not info._taskq_running_marked
        info._taskq_running_marked = True
        args = (coordinator, store, info.id, signal, info._taskq_generation or None, mark)
        if store is None or not type(admission).pump_off_loop:
