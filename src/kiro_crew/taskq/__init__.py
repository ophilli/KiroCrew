...
def open_default_store(
    ...
    *,
    ...
    adds nothing), reconcile (settle dead owners' rows), and only then may the
    caller dispatch. Raises :class:`TaskStoreUnavailable` when the file cannot
    be opened; import and reconcile failures are logged, never fatal, because
    a store that opens can still accept new work.
    ...
    try:
        reconcile_on_boot(store, artifact_probe=artifact_probe)
