...
# (``config/sections.py``): the daemon holds a queued stub for that long before
# a capacity refusal, and this budget is how long the stub keeps kiro-cli's
# transport open meanwhile. The stub reads no config, so the coupling is by
# value -- raising that key above 600 s needs this constant raised too; a
# lower key only refuses sooner. ``test_stub_reconnect_budget.py`` pins the
# equality, and >= _RECONNECT_CRASH_LOOP_CYCLES * _SUPERVISOR_RESPAWN_CYCLE_SECS.
