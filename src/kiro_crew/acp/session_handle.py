...
#: grow gateway memory through this set.
...
class AcpSessionHandle:
    ...
    def _handle_kas_subagent(self, update: dict) -> list[AcpEvent] | None:
        ...
        if is_parent:
            ...
            # parent, never charged — see native_child_sessions.
            self._note_native_child(subtask_id)
    ...
    def native_child_overflow(self) -> int:
        """Distinct native child ids seen past :data:`NATIVE_CHILD_ROSTER_CAP`
        this turn — counted, never stored."""
        return self._native_child_overflow

    def _note_native_child(self, child_sid: object) -> None:
        ...
        if not isinstance(child_sid, str) or not child_sid or len(child_sid) > 128:
            return
        if child_sid == self._session_id or child_sid in self._native_child_sids:
            return
    ...
    def _note_native_roster(self, subagents: object) -> None:
        ...
        Mirrors ``AcpRuntime._snapshot_subagent_sessions``' bounds."""
        if not isinstance(subagents, list):
            return
        for entry in subagents[:256]:
