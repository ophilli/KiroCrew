...
async def _hire_transaction(
    ...
    def _enroll(member_id: str, generation: str) -> None:
        ...
        # (enrolled_member_ids), written inside the create's publication with
        # the generation it minted; the create unwinds it if the row fails.
        agent_state.set_crewmate_record(
            member_id, generation=generation, template=agent, hired_at=hired_at
        )

    created = await _agents_handlers._create_crew(
        request, create_body, copy_source=agent, admit=_slug_collision_refusal, enroll=_enroll
    )
