...
| P5-e | Virtualized transcript for every non-page host: `chat-core/transcript/` (`VirtualTranscript` over `useVirtualChat` + `TranscriptScrollShell`, the page's row-identity builders as `rowKeys`, `useStableMessageKey`), mounted by `ChatMessageList` through its `transcript` prop; ChatPane, SideChat, ChatEmbed (and the spec-builder column through it) drop their own scroll div + `useChatScrollFollow`; ChatEmbed's poll is bounded to a page (`?limit=200`) with a load-earlier bar widening it to the handler ceiling. Stopgap for [#10005](https://github.com/kirodotdev/KiroCrew/issues/10005) lands separately in [#10026](https://github.com/kirodotdev/KiroCrew/pull/10026) | PR-2 of #10005 | in review |
