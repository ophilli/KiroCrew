...
in scope below. Two sites had to change —
`waiting_input`'s `enter_wait` (above) and `taskq_settle`'s `finish` (§ posted vs
inline) — and the rest are safe for a reason that is theirs, not the class's:
