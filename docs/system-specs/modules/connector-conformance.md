...
JSON retention form and the human-readable table below; the W00-S2 validator's
...
is not a reason to remove it. The human-readable table that follows is a
**courtesy** for readers and restates the same seven rows.
...
The human-readable table below is a courtesy restatement of the seven rows above.

| Responsibility | Owner | Concrete dependency (durable) | Later integration step | Current state |
|---|---|---|---|---|
| `ConformanceRun` structure (all required fields, types, timestamp, immutable `tested_sha`, `verdict` enum, `response_summary`) | W00-S2 connector-manifest validator, function `_validate_run` | shape defined in `connector-capability-manifest.md` §"Conformance and evidence: the structural contract lives here"; enforced by `_validate_run` in the W00-S2 validator | W00-S2 validator lands on `main` | specified in the merged spec; enforced by the W00-S2 validator, which is **not yet merged** |
| `EvidenceReceipt` structure (`receipt_id`, `conformance_run_ref`, `claim`, `runtime_verified`, `negative_test_refs`, cleanup fields) | W00-S2 connector-manifest validator, function `_check_receipt_shape` | shape defined in the same manifest section; enforced by `_check_receipt_shape` | W00-S2 validator lands on `main` | specified in the merged spec; enforced by the **not-yet-merged** W00-S2 validator |
| Per-effect cleanup rules (read=`not_applicable`; write/admin/share forbid `not_automatable`; `cleanup_confirmed` derived from `cleanup_status`) | W00-S2 validator, `_check_receipt_shape` per-effect branch | rules in `connector-capability-manifest.md` §"Per-effect verification and cleanup"; enforced by `_check_receipt_shape` | W00-S2 validator lands on `main` | specified in the merged spec; enforced by the **not-yet-merged** W00-S2 validator |
| Readback (`readback_result` null iff `effect: read`; non-read requires an independent-read object; `matched` must be true) | W00-S2 validator, `_check_receipt_shape` readback branch | rules in the manifest's receipt table and §"Per-effect verification and cleanup"; enforced by `_check_receipt_shape` | W00-S2 validator lands on `main` | specified in the merged spec; enforced by the **not-yet-merged** W00-S2 validator |
| Three-way reference identity (`verification_contract.receipt_ref` ↔ `ConformanceRun.evidence_receipt_ref` ↔ `EvidenceReceipt.conformance_run_ref`, by exact-string lookup) plus orphan-evidence and path-safety scans | W00-S2 validator, its three-way-equality check and whole-tree evidence scan | rule in `connector-capability-manifest.md` (receipt/run tables and §"Immutable ref binding"); enforced by the W00-S2 validator's equality and repo-wide scan | W00-S2 validator lands on `main` | specified in the merged spec; enforced by the **not-yet-merged** W00-S2 validator |
| Credential custody: a `ConformanceRun` binds to an authorized account/tenant via `account_binding_ref` and never carries a raw credential | kiro-cli owns the OAuth chain and token custody; Kiro Crew holds no connection credential | merged [`connections.md`](connections.md), its credential-boundary section; `account_binding_ref` defined in the manifest as "never a raw credential". The #9992 interface is **not** assumed (its diff is unread); a missing binding interface is reported as a named gap, never invented | W01 (shared control plane: binding/auth/policy), per the `W00→W01` DAG edge | boundary defined in merged `connections.md`; the concrete binding interface is pending W01 |
| Live-run dependency: a live conformance run producing a real `ConformanceRun`/`EvidenceReceipt` with `runtime_verified: true` | conformance runner (not yet implemented) + authorized-account evidence | `runtime_verified` stays false until a real live call. Real live capability depends on a later **W15** implementation plus authorized-account evidence. Contract §5.5 records that **no fixture-account owner has been designated** — an unassigned-owner fact, **not** a finding that accounts are externally unavailable | W00(4) evidence/runner/CI foundation, then W15 independent acceptance | no live run in this slice; runner unbuilt and fixture-account owner unassigned (contract §5.5) |

Nothing in the table above reads as discharged by this slice: rows 1–5 are
