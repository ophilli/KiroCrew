...
**Crewmate templates** (category id `Templates`; label `components.appstore.categories.templates`, named by what it offers so it does not collide with the drawer's *Agent template* row; `categories.ts::categoryFor(tags, manifest)` via `isTemplateOnlyApp`,
