...
- **Wakes.** Only `resume_grant` writes `running` (`wake_wait(to=running)`,
  ...
  back. `resume_reserve` reserves nothing while gateway admission is CLOSED (the
...
- **Cancel.** `_unqueue_impl` cancels the store row FIRST (`taskq_cancel_queued`,
  ...
  order (see taskq.md § Atomic claim).
...
- **Refill (`taskq_refill_window`).** Every lane with a store row waiting gets
  ...
  in FIFO order — never a resume entry, never a lane's last entry), then the
  remaining room is filled by `fetch_dispatchable_fair`. Two scheduler
